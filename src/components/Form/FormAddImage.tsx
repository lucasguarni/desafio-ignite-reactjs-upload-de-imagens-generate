...
export function FormAddImage({ closeModal }: FormAddImageProps): JSX.Element {
    ...
    image: {
      ...
      validate: {
        lessThan10MB: (v: FileList) => {
          const sizeMb = (Math.round(v[0].size / 1000) / 1000);
          if(sizeMb >= 10) {
            return 'O arquivo deve ser menor que 10MB';
          }
        },
        acceptedFormats: (v: FileList) => {
          const type = v[0].type.toLocaleLowerCase();
          const regex = new RegExp(/(\/|\.)(jpeg|jpg|png|gif)/ig);
  ...
  const onSubmit = async (data: Record<string, unknown>): Promise<void> => {
    try {
      ...
      const response = await mutation.mutateAsync({url: imageUrl, title: String(data.title), description: String(data.description)});
      if (response) {
        toast({
